...
router.post('/edit/:user_id/', function(req, res){
  ...
  .then(function(){
    ...
    employee
      ...
      // Update carried over allowance if necessary
      .then(() => {
...
function get_and_validate_user_parameters(args) {
    ...
    if (carried_over_allowance && ! validator.isFloat(carried_over_allowance) ) {
      req.session.flash_error(
        'Carried over allowance of '+item_name+' should be a valid number'
      );
    } else if (carried_over_allowance && ! ( carried_over_allowance % 1 === 0 || Math.abs( carried_over_allowance % 1 ) === 0.5 )) {
      req.session.flash_error(
        'Carried over allowance of '+item_name+' should be either whole integer number or with half'
      );
    }
