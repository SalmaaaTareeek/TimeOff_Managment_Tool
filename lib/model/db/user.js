...
function get_instance_methods(sequelize) {
  ...
  return {
    ...
    get_reset_password_token : function(){
      var self = this;
      return new Buffer.alloc( (self.email + ' ' + self.Model.hashify_password( self.password )).length,(self.email + ' ' + self.Model.hashify_password( self.password ))) .toString('base64');
    },
